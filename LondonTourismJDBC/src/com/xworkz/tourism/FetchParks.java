...
public class FetchParks {
	...
	private static void fetchName() {
		...
		String fetchQuery = "SELECT * FROM parks";
		...
		try {
			...
			int j = stm.executeUpdate(updateQuery);
			System.out.println(j);
			int k = stm.executeUpdate(deleteQuery);
			System.out.println(k);
			
			ResultSet rs = stm.executeQuery(fetchQuery);
			
			while (rs.next()) {
				System.out.println(rs.getString("park_name"));
	...
	private static void fetchEmpCount() {
		...
		String fetchQuery = "SELECT * FROM factory_info";
